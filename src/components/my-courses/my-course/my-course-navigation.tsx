...
export const MyCourseNavigation = ({ stepNumber, course }: MyCourseNavigationProps) => {
  const activeStepNumber = Number(stepNumber);
  return (
        ...
        <ul className="flex w-max min-w-full lg:min-w-0 gap-2 rounded-xl border border-zinc-800 bg-zinc-900/50 p-1 backdrop-blur-sm">
          {course.steps.map((step) => {
            ...
            return (
              <li key={step.number}>
                <Link
                  href={`/meu-curso/${course.slug}/etapas/${step.number}`}
                  data-active={isActive}
                  className={`relative px-4 py-2 text-sm rounded-lg transition-colors whitespace-nowrap border border-transparent hover:bg-zinc-800/70 ${
