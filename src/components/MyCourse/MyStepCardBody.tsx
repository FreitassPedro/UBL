    ...
    return (
        <div>
            {step.cadeiras.map((cadeira) => (
                <Link to={`/curso/${cadeira.id}`} key={cadeira.id} className="border-t border-gray-600 last:border-b-0 cursor-pointer">
                    <div className="px-8 py-6 space-y-4 bg-white/5 border border-white/10 hover:bg-black/60 transition-all duration-300 ">
