import { BookRecommendation } from "@/components/features/curriculum/BookRecommendation";
import { Button } from "@/components/ui/button";
import { Asterisk, BookOpen, ChevronDown, ChevronUp } from "lucide-react";
import { useState } from "react";

interface SemesterSubjectItem {
  subject: {
    name: string;
...
export const SemesterSubjectItem = ({
  subject,
  index,
}: SemesterSubjectItem) => {
  ...
  return (
    <li key={index} className="relative group">
      <div
        className={`relative ${getColorBackground()} border border-white/10 hover:border-white/20 px-4 py-2 transition-all duration-300 hover:scale-[1.01] hover:shadow-2xl`}
      >
        {/* Content Container */}
        <div className="flex justify-between items-center">
          <div className="flex-1">
            ...
            <div className="flex flex-wrap items-center gap-2 mt-2">
              <div
                className={`flex items-center gap-1 px-3 py-1 ${subject.prerequisites.length > 0 ? "from-purple-400/20 to-pink-600/20" : ""} bg-linear-to-r  rounded-full  border-white/20`}
              >
              ...
              {subject.prerequisites.length > 0 && (
                <ul className="flex flex-wrap gap-2">
                  {subject.prerequisites.map((prereq, i) => (
                    <li
                      key={i}
                      className="text-xs bg-linear-to-r from-purple-500/10 to-pink-800/20 text-purple-200 px-2 py-1 rounded-full border border-purple-500/30"
                    >
                      {prereq}
                    </li>
                  ))}
