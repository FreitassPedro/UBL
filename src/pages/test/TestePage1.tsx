...
const SyllabusItem = ({ lesson }: { lesson: Lesson }) => {
  ...
  return (
    <li
      className={cn(
        "flex items-center py-3.5 border-b border-white/5 cursor-pointer transition-colors group",
        lesson.status,
      )}
    >
      <div
        className={cn(
          "w-[18px] h-[18px] rounded-full mr-3 flex items-center justify-center text-xs",
          statusClass,
        )}
      >
        {statusContent && (
          <span className="text-black text-[10px]">{statusContent}</span>
        )}
      </div>
      <span className={cn("text-sm grow", titleClass)}>{lesson.title}</span>
    ...
    <div className="min-h-screen flex items-center justify-center bg-bg-body text-text-main font-inter overflow-hidden p-6">
      <div className="w-full max-w-6xl h-[85vh] grid grid-cols-1 lg:grid-cols-[2.5fr_1fr] gap-6">
        ...
        <main className="flex flex-col p-8 bg-bg-card border border-bg-hover rounded-xl shadow-2xl shadow-black/40 overflow-hidden">
          ...
          <h1 className="font-light text-2xl tracking-tight mb-2">
            Advanced Typography Systems
          </h1>
          <h2 className="font-normal text-lg text-text-muted mb-6 tracking-tight">
            Understanding vertical rhythm and modular scales in interface
            design.
          </h2>

          <p className="font-light leading-relaxed text-zinc-300-light">
            In this module, we move beyond basic font selection. We will explore
            how to set up a mathematical type scale that ensures consistency
            across your application. We will also cover the implementation of
            variable fonts and how to optimize them for dark mode environments.
          </p>
        </main>
        ...
        <aside className="flex flex-col gap-6 p-8 bg-bg-card border border-bg-hover rounded-xl shadow-2xl shadow-black/40">
          ...
          <div className="flex items-center justify-between pb-6 border-b border-border-subtle">
            <div className="flex flex-col">
              <h4 className="text-5xl font-extralight text-text-main leading-none">
                {currentProgress}%
              </h4>
              <span className="text-sm text-text-muted font-light mt-1">
                Course Completed
              </span>
            </div>
          ...
          <div>
            <h3 className="font-medium text-xs uppercase tracking-wider text-text-muted mb-4">
              Weekly Activity
            </h3>
            <div className="flex justify-between items-end h-[100px] mt-2 px-2">
              {activityData.map((data) => (
                ...
                >
                  <div
                    className={cn(
                      "w-1.5 rounded-t-sm transition-all duration-500 ease-in-out",
                      data.isActive
                        ? "bg-accent-primary shadow-[0_0_10px_var(--tw-colors-accent-glow-bg)]"
                        : "bg-bg-hover",
                    )}
          ...
          <div className="flex flex-col h-full overflow-hidden">
            <h3 className="font-medium text-xs uppercase tracking-wider text-text-muted mb-2">
              Next Up
            </h3>
